  ...
  return (
    <Fragment>
      ...
      <Box className={styles.card__item__container}>
        {Item1.items.map((data) => {
          return (
            <Box sx={{ padding: "3px", textAlign: "left" }} key={data.id}>
              <Typography
                sx={{ color: "#4D505A" }}
                fontSize="12px"
                lineHeight="16px"
      ...
      <Card sx={{ backgroundColor: "#F3F5F7" , width: "fit-content", padding: 0.75, marginTop: "5px" }}>
...
export default function BaseActionCard({ children }: Props) {
  return (
